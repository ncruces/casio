...
var ASM_CONSTS = {
  79784: () => { return Module['suspended']; },  
 79816: ($0) => { Module['suspended'] = $0; },  
 79846: () => { var len = lengthBytesUTF8(tx) + 1; var s = _malloc(len); stringToUTF8(tx, s, len); return s; },  
 79943: () => { tx = ""; },  
 ...
 80117: () => { return new Date().getTimezoneOffset() * 60 * 1000; },  
 80172: ($0) => { const date = new Date(Date.now() + $0); return date.getSeconds() | (date.getMinutes() << 6) | (date.getHours() << 12) | (date.getDate() << 17) | ((date.getMonth() + 1) << 22) | ((date.getFullYear() - 2020) << 26); },  
 80390: () => { return performance.now(); },  
 80420: () => { return performance.now(); },  
 80450: () => { document.getElementById("custom").style.display = ""; },  
 80506: () => { document.getElementById("classic").style.display = "none"; },  
 80567: () => { document.getElementById("classic").style.display = "none"; },  
 80628: () => { document.getElementById("custom").style.display = "none"; },  
 80688: ($0, $1) => { document.querySelectorAll("[data-com='" + $0 + "'][data-seg='" + $1 + "']") .forEach((e) => e.style.opacity = 1); },  
 80806: ($0, $1) => { document.querySelectorAll("[data-com='" + $0 + "'][data-seg='" + $1 + "']") .forEach((e) => e.style.opacity = 0); },  
 80924: () => { document.querySelectorAll("[data-com][data-seg]") .forEach((e) => e.style.opacity = 0); },  
 81016: () => { if (!Module['audioContext']) { Module['audioContext'] = new (window.AudioContext || window.webkitAudioContext)(); } },  
 81136: ($0) => { const audioContext = Module['audioContext']; if (!audioContext) return; if (!(audioContext._oscillator && audioContext._gain)) { const oscillator = audioContext.createOscillator(); const gain = audioContext.createGain(); oscillator.type = 'triangle'; oscillator.connect(gain); gain.connect(audioContext.destination); oscillator.start(0); audioContext._oscillator = oscillator; audioContext._gain = gain; } audioContext._oscillator.frequency.value = 1e6/$0; audioContext._gain.gain.value = volumeGain; },  
 81641: () => { const audioContext = Module['audioContext']; if (audioContext && audioContext._gain) { audioContext._gain.gain.value = 0; } },  
 81769: ($0, $1, $2) => { let filter = document.getElementById("ledcolor"); let color_matrix = filter.children[0].values.baseVal; color_matrix[0].value = $0 / 255; color_matrix[6].value = $1 / 255; color_matrix[12].value = $2 / 255; document.getElementById('light').style.opacity = Math.min(255, $0 + $1 + $2) / 255; },  
 82064: () => { return lat; },  
 82080: () => { return lon; },  
 82096: () => { return temp_c || 25.0; },  
 82123: () => { return -new Date().getTimezoneOffset(); }
};
